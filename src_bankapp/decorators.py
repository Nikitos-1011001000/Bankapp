...
def log(filename: Optional[str] = None):
    ...
    для логирования.
    func(Callable): оригинальная функция, которую
    нужно прологировать.
    Возвращает:
    Callable: версия функции
    с логированием.
    """
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            kwargs_str = ', '.join(f"{k}={vr}" for k, v in kwargs.items())
            all_args = ', '.join(filter(None, [args_str, kwargs_str]))

            try:
                result = func(*args, **kwargs)
                log_message += f"{func_name} завершена успешно. Результат: {result}\n"
                _write_log(log_message, filename)
                return result
            except Exception as e:
                log_message += (
                    f"{func_name} вызвала ошибку {type(e).__name__}: {e}. "
